...
/**
 ...
 value< root.Val   go to left sub algorithms.tree
 else go to right sub algorithms.tree

 if the target node is leaf , delete it.
 if target has both left and right subtree, find the rightest(biggest) node in left sub algorithms.tree to place the position where target node is
 if the target has no left sub algorithms.tree, parent point to the right sub algorithms.tree
    ...
    public TreeNode removeNode(TreeNode root, int value) {
        ...
        while(root!=null){
            if(root.val == value) {
                ...
                if(root.left!=null && root.right!=null ){ // look for the rightest(biggest) leaf in sub left algorithms.tree
                    temp = root.left;
